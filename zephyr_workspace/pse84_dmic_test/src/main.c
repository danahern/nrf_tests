			   ...
			   size_t block_count)
{
	...
	for (int i = 0; i < block_count; ++i) {
		...
		for (size_t j = 0; j < n; j++) {
			int32_t a = samples[j] < 0 ? -samples[j] : samples[j];
			if (a > peak) peak = a;
			acc += a;
			if (a > 5) nonzero++;
		}
		LOG_INF("%d - buffer %p %u bytes | first8=%d,%d,%d,%d,%d,%d,%d,%d | "
			"peak=%ld mean_abs=%ld nonzero(|s|>5)=%u/%u",
			i, buffer, size,
			samples[0], samples[1], samples[2], samples[3],
			samples[4], samples[5], samples[6], samples[7],
			(long)peak, (long)(acc / (long)n),
			(unsigned)nonzero, (unsigned)n);
